//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:game_collection_client/api.dart';
import 'package:test/test.dart';

// tests for GameWithLogsExtendedDTO
void main() {
  // final instance = GameWithLogsExtendedDTO();

  group('test GameWithLogsExtendedDTO', () {
    // DateTime addedDatetime
    test('to test the property `addedDatetime`', () async {
      // TODO
    });

    // bool backup
    test('to test the property `backup`', () async {
      // TODO
    });

    // String coverFilename
    test('to test the property `coverFilename`', () async {
      // TODO
    });

    // String coverUrl
    test('to test the property `coverUrl`', () async {
      // TODO
    });

    // String edition
    test('to test the property `edition`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // List<GameLogDTO> logs (default value: const [])
    test('to test the property `logs`', () async {
      // TODO
    });

    // GameLogDTO longestSession
    test('to test the property `longestSession`', () async {
      // TODO
    });

    // GameStreakDTO longestStreak
    test('to test the property `longestStreak`', () async {
      // TODO
    });

    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // String notes
    test('to test the property `notes`', () async {
      // TODO
    });

    // int rating
    test('to test the property `rating`', () async {
      // TODO
    });

    // int releaseYear
    test('to test the property `releaseYear`', () async {
      // TODO
    });

    // String saveFolder
    test('to test the property `saveFolder`', () async {
      // TODO
    });

    // String screenshotFolder
    test('to test the property `screenshotFolder`', () async {
      // TODO
    });

    // GameStatus status
    test('to test the property `status`', () async {
      // TODO
    });

    // List<GameStreakDTO> streaks (default value: const [])
    test('to test the property `streaks`', () async {
      // TODO
    });

    // String totalTime
    test('to test the property `totalTime`', () async {
      // TODO
    });

    // DateTime updatedDatetime
    test('to test the property `updatedDatetime`', () async {
      // TODO
    });


  });

}
