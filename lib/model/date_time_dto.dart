...
class DateTimeDTO {
  ...
  static DateTimeDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'datetime'), 'Required key "DateTimeDTO[datetime]" is missing from JSON.');
        assert(json[r'datetime'] != null, 'Required key "DateTimeDTO[datetime]" has a null value in JSON.');
        return true;
      }());
