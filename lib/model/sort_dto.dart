...
class SortDTO {
  ...
  static SortDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'field'), 'Required key "SortDTO[field]" is missing from JSON.');
        assert(json[r'field'] != null, 'Required key "SortDTO[field]" has a null value in JSON.');
        assert(json.containsKey(r'order'), 'Required key "SortDTO[order]" is missing from JSON.');
        assert(json[r'order'] != null, 'Required key "SortDTO[order]" has a null value in JSON.');
        return true;
      }());
