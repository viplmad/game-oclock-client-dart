...
class UserSearchResult extends SearchResultDTO<UserDTO> {
  /// Returns a new [UserSearchResult] instance.
  UserSearchResult({
    List<UserDTO> data = const [],
    required int page,
    required int size,
  ...
  @override
  String toString() => 'UserSearchResult[data=$data, page=$page, size=$size]';
  ...
  static Map<String, List<UserSearchResult>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UserSearchResult.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
