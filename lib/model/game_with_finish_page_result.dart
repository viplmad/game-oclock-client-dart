...
class GameWithFinishPageResult extends PageResultDTO<GameWithFinishDTO> {
  ...
  @override
  String toString() => 'GameWithFinishPageResult[data=$data, page=$page, size=$size]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GameWithFinishPageResult[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GameWithFinishPageResult[$key]" has a null value in JSON.');
        });
        return true;
      }());
